...
## 14. Research index & plan amendments (2026-08-06)

Deep-research documents now govern their areas; where they conflict with the sections above, **the research documents win**:

- **`ULTRAPROMPT.md`** — the master build contract for the coding agent: mission, POV directive, hard rules, edge-case register (every item must be handled or explicitly deferred), build order with user-POV acceptance tests.
- **`research/company-universe.md`** — how to assemble the 100–500 company target list (Ru'ya exhibitors, ADX/DFM, CBUAE institutions, Nafis Award/MoU press sweeps, semi-gov portfolios). Amendments: **free-zone-only entities are exempt from MoHRE quotas → hard exclusion filter**; rank by hiring *propensity*, not quota pressure; government entities go in a separate segment.
- **`research/people-discovery.md`** — ranked senior and junior/mid contact-finding playbooks. Amendments to §3: the **company Emiratisation / "meet our people" page becomes the first Tier-2 check**; mid-level path = job-ad title extraction → `site:ae.linkedin.com` SERP resolution with a **mandatory geography check**; DMCC directory is ToS-prohibited for outreach — never use; add a permanent **suppression list** table (PDPL right-to-object).
